...
class PenerjemahanDataset implements StatistikDataset
{
    ...
    public function perBulan(?string $from, ?string $to, ?string $mode = null): Collection
    {
        return $this->baseQuery($from, $to)
            ->selectRaw('DATE_FORMAT(penerjemahans.created_at, "%Y-%m") as periode, COUNT(*) as total')
            ->groupBy('periode')
            ->orderBy('periode')
            ->pluck('total', 'periode');
    }
    ...
    public function perHari(?string $from, ?string $to, ?string $mode = null): Collection
    {
        return $this->baseQuery($from, $to, $mode)
            ->selectRaw('DATE_FORMAT(penerjemahans.created_at, "%Y-%m-%d") as periode, COUNT(*) as total')
            ->groupBy('periode')
            ->orderBy('periode')
            ->pluck('total', 'periode');
    }
    ...
    public function perProdi(?string $from, ?string $to, ?string $mode = null): Collection
    {
        $rows = $this->baseQuery($from, $to)
            ->join('users', 'users.id', '=', 'penerjemahans.user_id')
    ...
    {
        $query = Penerjemahan::query();

        if ($from) {
            $query->where('penerjemahans.created_at', '>=', $from . ' 00:00:00');
        }
        if ($to) {
            $query->where('penerjemahans.created_at', '<=', $to . ' 23:59:59');
        }
