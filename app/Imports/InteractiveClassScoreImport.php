...
class InteractiveClassScoreImport
{
    private const DEFAULT_STUDY_PROGRAM = 'Pendidikan Bahasa Inggris';
    private const DEFAULT_ARABIC_STUDY_PROGRAM = null;
    ...
    {
        $this->resetState();
        $this->processFile($filePath, $defaultYear, false, $track, $defaultStudyProgram);
    ...
    ): array
    {
        ...
        $this->processFile($filePath, $defaultYear, true, $track, $defaultStudyProgram);
    ...
    private function resolveDefaultStudyProgram(string $track, ?string $defaultStudyProgram): ?string
    {
        if (filled($defaultStudyProgram)) {
            return trim((string) $defaultStudyProgram);
        }

        return $track === InteractiveClassScore::TRACK_ARABIC
            ? self::DEFAULT_ARABIC_STUDY_PROGRAM
            : self::DEFAULT_STUDY_PROGRAM;
    }
    ...
    private function bootstrapUserLookup(): void
    {
        $this->usersBySrn = User::query()
            ->whereNotNull('srn')
            ->get(['id', 'srn'])
            ->groupBy(fn (User $user): ?string => InteractiveClassScores::normalizeSrn($user->srn))
    ...
    ): void
    {
        ...
        $defaultStudyProgram = $this->resolveDefaultStudyProgram($track, $defaultStudyProgram);
        ...
        if (! isset($map['score']) || ! isset($map['semester'])) {
            fclose($handle);
            throw new RuntimeException('Kolom AVERAGE/NILAI dan SEMESTER wajib ada di CSV.');
        }
        ...
        while (($row = fgetcsv($handle, 0, $delimiter, '"', '\\')) !== false) {
            ...
            $payload = $this->extractPayload($row, $map, $sourceYear, $sourceFile, $track, $defaultStudyProgram);
            if ($payload === null) {
            ...
            /** @var InteractiveClassScore $record */
    ...
    private function mapHeaders(array $header): array
    {
        ...
        foreach ($header as $index => $column) {
            ...
            if (in_array($normalized, ['SRN', 'NPM', 'NIM'], true)) {
                $map['srn'] = $index;
                continue;
            }

            if (in_array($normalized, ['NAMA', 'NAME', 'NAMA MAHASISWA', 'STUDENT NAME'], true)) {
                $map['name'] = $index;
                continue;
            }
            ...
            if (in_array($normalized, ['JENIS', 'TRACK', 'TYPE', 'JENIS INTERACTIVE', 'INTERACTIVE TYPE'], true)) {
                $map['track'] = $index;
                continue;
            }

            if (in_array($normalized, ['AVERAGE', 'RATA RATA', 'RATA-RATA', 'NILAI', 'NILAI AKHIR'], true)) {
                $map['score'] = $index;
                continue;
            }

            if (in_array($normalized, ['HURUF MUTU', 'GRADE', 'ALPHABETICAL'], true)) {
                $map['grade'] = $index;
                continue;
            }

            if (in_array($normalized, ['TAHUN', 'YEAR', 'SOURCE YEAR'], true)) {
                $map['source_year'] = $index;
                continue;
            }

            if (in_array($normalized, ['SEMESTER'], true)) {
                $map['semester'] = $index;
                continue;
            }
            ...
            if (in_array($normalized, ['NO ABSEN', 'NOABSEN', 'ABSEN'], true)) {
            ...
            if (in_array($normalized, ['LISTENING', 'SPEAKING', 'READING', 'WRITING', 'PHONONLOGY', 'STRUCTURE', 'VOCABULARY', 'TOTAL', 'BULAN', 'PREDICATE'], true)) {
                $map[strtolower(str_replace(' ', '_', $normalized))] = $index;
            }
        }
    ...
    ): ?array
    {
        ...
        $track = isset($map['track']) ? trim((string) ($row[$map['track']] ?? '')) : null;
        $track = InteractiveClassScores::normalizeTrack($track ?: $defaultTrack);
        ...
        $sourceYear = isset($map['source_year'])
            ...
            : $defaultYear;
        $semester = isset($map['semester']) ? $this->parseSemester($row[$map['semester']] ?? null, $track) : null;

        $meta = [
            'imported_via' => 'csv',
            'group' => isset($map['group']) ? trim((string) ($row[$map['group']] ?? '')) : null,
            'attendance_no' => isset($map['attendance_no']) ? trim((string) ($row[$map['attendance_no']] ?? '')) : null,
            'student_no' => isset($map['student_no']) ? trim((string) ($row[$map['student_no']] ?? '')) : null,
            'listening' => isset($map['listening']) ? $this->parseScore($row[$map['listening']] ?? null) : null,
            'speaking' => isset($map['speaking']) ? $this->parseScore($row[$map['speaking']] ?? null) : null,
            'reading' => isset($map['reading']) ? $this->parseScore($row[$map['reading']] ?? null) : null,
            'writing' => isset($map['writing']) ? $this->parseScore($row[$map['writing']] ?? null) : null,
            'phononology' => isset($map['phononology']) ? $this->parseScore($row[$map['phononology']] ?? null) : null,
            'structure' => isset($map['structure']) ? $this->parseScore($row[$map['structure']] ?? null) : null,
            'vocabulary' => isset($map['vocabulary']) ? $this->parseScore($row[$map['vocabulary']] ?? null) : null,
            'total' => isset($map['total']) ? $this->parseScore($row[$map['total']] ?? null) : null,
            'month' => isset($map['bulan']) ? trim((string) ($row[$map['bulan']] ?? '')) : null,
            'predicate' => isset($map['predicate']) ? trim((string) ($row[$map['predicate']] ?? '')) : null,
        ];

        return [
            'srn' => $srn ?: null,
            'srn_normalized' => InteractiveClassScores::normalizeSrn($srn),
            'name' => $name ?: null,
            'name_normalized' => InteractiveClassScores::normalizeName($name),
            'study_program' => $studyProgram ?: $defaultStudyProgram,
            'track' => $track,
            'semester' => $semester,
     ...
     * @return array{status: string, reason?: string, identity_key?: string|null, record?: InteractiveClassScore, conflict_context?: array<string, mixed>}
     */
    private function inspectPayload(array $payload): array
    {
        $track = InteractiveClassScores::normalizeTrack($payload['track'] ?? null);

        if (! is_numeric($payload['score'] ?? null)) {
            return [
                'status' => 'skip',
                'reason' => 'Nilai average kosong atau tidak valid',
            ];
        }

        if ((float) $payload['score'] <= 0) {
            return [
                'status' => 'skip',
                'reason' => 'Tidak lulus (nilai 0/kosong)',
            ];
        }

        if ($this->isFailedPayload($payload)) {
            return [
                'status' => 'skip',
                'reason' => 'Tidak lulus (grade E / predicate Fail)',
            ];
        }

        if (! is_numeric($payload['semester'] ?? null)) {
            return [
                'status' => 'skip',
                'reason' => $track === InteractiveClassScore::TRACK_ARABIC
                    ? 'Tahap Bahasa Arab kosong atau tidak valid'
                    : 'Semester kosong atau tidak valid',
            ];
        }

        $semester = (int) $payload['semester'];
        if ($semester < 1 || $semester > InteractiveClassScores::maxSemester($track)) {
            return [
                'status' => 'skip',
                'reason' => $track === InteractiveClassScore::TRACK_ARABIC
                    ? 'Bahasa Arab hanya 1 sampai 2'
                    : 'Semester harus antara 1 sampai 6',
            ];
        }
        ...
        if ($identityKey === null) {
            return [
                'status' => 'skip',
                'reason' => 'Nama tanpa SRN wajib disertai tahun data dan semester',
            ];
        }
        ...
        if (filled($payload['srn_normalized'])) {
            return [
                ...
                'record' => InteractiveClassScore::query()->firstOrNew([
                    'track' => $track,
        ...
        if ($matches > 1) {
            $existingMatches = (clone $query)
                ->orderBy('id')
                ->limit(5)
                ->get(['id', 'track', 'srn', 'name', 'study_program', 'source_year', 'semester', 'score'])
                ->map(fn (InteractiveClassScore $row): array => [
                    'id' => $row->id,
                    'track' => $row->track,
                    'srn' => $row->srn,
                    'name' => $row->name,
            ...
            return [
                'status' => 'conflict',
                'reason' => 'Data existing ambigu (nama/tahun/semester)',
                'conflict_context' => [
        ...
        return [
            'status' => 'ok',
            'identity_key' => $identityKey,
            'record' => $query->first() ?? new InteractiveClassScore(),
        ];
    }
    ...
    {
        $semester = is_numeric($payload['semester'] ?? null) ? (int) $payload['semester'] : null;
        if (! $semester) {
            return null;
        }
        ...
        if (is_string($srnNormalized) && $srnNormalized !== '') {
            return 'track:' . InteractiveClassScores::normalizeTrack($payload['track'] ?? null)
                . '|srn:' . $srnNormalized
                . '|semester:' . $semester;
        }
        ...
        return 'identity:' . implode('|', [
            InteractiveClassScores::normalizeTrack($payload['track'] ?? null),
            $nameNormalized,
    ...
    private function parseSemester(mixed $value, ?string $track = null): ?int
    {
        if ($value === null) {
            return null;
        }
        ...
        $maxSemester = InteractiveClassScores::maxSemester(InteractiveClassScores::normalizeTrack($track));

        return ($semester >= 1 && $semester <= $maxSemester) ? $semester : null;
    }

    /** @param array<string, mixed> $payload */
    private function isFailedPayload(array $payload): bool
    {
        $grade = mb_strtoupper(trim((string) ($payload['grade'] ?? '')), 'UTF-8');
        if ($grade === 'E') {
            return true;
        }

        $predicate = mb_strtoupper(trim((string) data_get($payload, 'meta.predicate', '')), 'UTF-8');

        return str_contains($predicate, 'FAIL');
    }
    ...
    /** @param array<string, mixed> $payload */
    private function storeReadySample(int $rowNumber, array $payload, string $mode, ?InteractiveClassScore $record = null): void
    {
        ...
        $this->sampleReadyRows[] = [
            ...
            'name' => $payload['name'] ?? null,
            'study_program' => $payload['study_program'] ?? null,
            'track' => $payload['track'] ?? InteractiveClassScore::TRACK_ENGLISH,
            'source_year' => $payload['source_year'] ?? null,
            'semester' => $payload['semester'] ?? null,
            'score' => $score,
    ...
    /** @param array<string, mixed> $payload */
    private function storeUpdateDetail(int $rowNumber, array $payload, InteractiveClassScore $record): void
    {
        $this->updateDetails[] = [
            'row' => $rowNumber,
            ...
            'incoming_score' => is_numeric($payload['score'] ?? null)
                ? (int) round((float) $payload['score'])
                : null,
            'incoming_grade' => $payload['grade'] ?? null,
        ];
    }

    private function syncMatchingUsers(InteractiveClassScore $record): void
    {
        $srn = $record->srn_normalized;
        if ($srn === null || ! is_numeric($record->score) || ! is_numeric($record->semester)) {
            return;
        }
        ...
        foreach ($users as $user) {
            if (! isset($this->syncedUserSnapshots[$user->id])) {
                $this->syncedUserSnapshots[$user->id] = [
                    'id' => $user->id,
                    'interactive_class_1' => $user->interactive_class_1,
                    'interactive_class_2' => $user->interactive_class_2,
                    'interactive_class_3' => $user->interactive_class_3,
                    'interactive_class_4' => $user->interactive_class_4,
                    'interactive_class_5' => $user->interactive_class_5,
                    'interactive_class_6' => $user->interactive_class_6,
                    'interactive_bahasa_arab_1' => $user->interactive_bahasa_arab_1,
                    'interactive_bahasa_arab_2' => $user->interactive_bahasa_arab_2,
                ];
            }

            InteractiveClassScores::applyScoreToUser($user, (string) $record->track, (int) $record->semester, (float) $record->score);
            $this->syncedUsers++;
        }
    }
}
